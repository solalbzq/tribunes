  ...
  return (
    <div className="space-y-4 pt-2 border-t border-gray-100">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">Score mi-temps (nous)</label>
        ...
        <div>
          <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">Score mi-temps (eux)</label>
          <input type="number" min="0" placeholder="0"
      ...
      <div>
        <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">Buteurs <span className="font-normal text-gray-400">(optionnel)</span></label>
      ...
      <div>
        <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">Cartons <span className="font-normal text-gray-400">(optionnel)</span></label>
...
function TennisFields({ extra, set }: { extra: Record<string, unknown>; set: (k: string, v: unknown) => void }) {
  return (
    <div className="space-y-4 pt-2 border-t border-gray-100">
      <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Détails 🎾</p>
      <div>
        <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">Détail des sets</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">Format</label>
        ...
        <div>
          <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">Catégorie</label>
  ...
  return (
    <div className="space-y-4 pt-2 border-t border-gray-100">
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-3">
        {quarters.map(q => (
          <div key={q.key} className="space-y-1">
            <label className="text-xs font-semibold text-[#1a1a2e]">{q.label}</label>
            <div className="flex items-center gap-2">
              <input type="number" min="0" placeholder="0"
                value={(extra[`${q.key}Home`] as string) ?? ''}
                onChange={e => set(`${q.key}Home`, Number(e.target.value))}
                className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm text-center focus:outline-none focus:ring-2 focus:ring-[#e94560]/30" />
      ...
      <div className="space-y-1">
        <label className="text-xs font-semibold text-[#1a1a2e]">Prolongation <span className="font-normal text-gray-400">(si applicable)</span></label>
        <div className="flex items-center gap-2">
          <input type="number" min="0" placeholder="0"
    ...
    <div className="space-y-3 pt-2 border-t border-gray-100">
      ...
      {sets.map(s => (
        <div key={s.key} className="space-y-1">
          <label className="text-xs font-semibold text-[#1a1a2e]">{s.label} {!s.required && <span className="font-normal text-gray-400">(optionnel)</span>}</label>
          <div className="flex items-center gap-2">
            <input type="number" min="0" placeholder="0"
...
function HandballFields({ extra, set }: { extra: Record<string, unknown>; set: (k: string, v: unknown) => void }) {
  return (
    <div className="space-y-4 pt-2 border-t border-gray-100">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">Mi-temps (nous)</label>
          <input type="number" min="0" placeholder="0"
            value={(extra.halfTimeHome as string) ?? ''}
            onChange={e => set('halfTimeHome', Number(e.target.value))}
            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#e94560]/30" />
        ...
        <div>
          <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">Mi-temps (eux)</label>
          <input type="number" min="0" placeholder="0"
            value={(extra.halfTimeAway as string) ?? ''}
            onChange={e => set('halfTimeAway', Number(e.target.value))}
            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#e94560]/30" />
      ...
      <div>
        <label className="text-xs font-semibold text-[#1a1a2e] block mb-1">7 mètres <span className="font-normal text-gray-400">(optionnel)</span></label>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-8 max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        {/* Domicile / Extérieur */}
        <div>
          <label className="block text-sm font-semibold text-[#1a1a2e] mb-2">Où jouiez-vous ?</label>
        ...
        {/* Adversaire */}
        <div>
          <label className="block text-sm font-semibold text-[#1a1a2e] mb-1">Adversaire</label>
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 text-center">
              ...
              <input type="number" min="0" required
                ...
                className="w-full border border-gray-200 rounded-xl px-4 py-3 text-2xl font-bold text-center focus:outline-none focus:ring-2 focus:ring-[#e94560]/30"
                placeholder="0" />
            </div>
          ...
          {hasScore && (
            <p className={`text-center text-sm font-bold mt-2 ${
              myScore > oppScore ? 'text-[#10b981]' :
              myScore < oppScore ? 'text-[#e94560]' : 'text-gray-400'
            }`}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-semibold text-[#1a1a2e] mb-1">Compétition</label>
            <input type="text" value={competition} onChange={e => setCompetition(e.target.value)}
              className="w-full border border-gray-200 rounded-xl px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#e94560]/30"
              placeholder="Championnat D2..." />
          </div>
          <div>
            <label className="block text-sm font-semibold text-[#1a1a2e] mb-1">Date</label>
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            <div onClick={() => fileRef.current?.click()}
              className="w-20 h-20 rounded-xl border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden cursor-pointer hover:border-[#e94560] transition shrink-0">
              {photoPreview
        ...
        <div>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2}
            className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#e94560]/30 resize-none"
            placeholder={
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={loading || loadingVisual}
            className="flex-1 bg-[#e94560] text-white font-bold py-3.5 rounded-xl hover:bg-[#d63a52] transition disabled:opacity-60 flex items-center justify-center gap-2">
          ...
          <button type="button" onClick={handleVisualOnly}
            ...
            className="px-4 py-3.5 bg-[#1a1a2e] text-white font-bold rounded-xl hover:bg-[#2a2a4e] transition disabled:opacity-40 text-sm whitespace-nowrap">
            🖼️ Visuel seul
          </button>
        </div>
      </form>
